...
export default function Page({ params }: { params: { productid: string } }) {
  ...
  const form = useForm<editSchemaType>({
    ...
    defaultValues: async () => {
      ...
      const response: any = await getSlotData(productId);
    ...
    <div className=" w-full flex flex-col items-start h-full  flex-1  ">
      <SubSectionWrap isLoading={loading}>
        <div className="w-full   flex-1 flex ">
          <div className="p-6 flex-1 grid grid-cols-12 gap-6 w-full  ">
            <Form {...form}>
              <form
                className="w-full  gap-6 col-span-12   grid grid-cols-12 bg-white border p-6"
                onSubmit={onSubmit}
              >
                <div className=" col-span-6 grid grid-cols-12 gap-6 w-full  pb-12 ">
            ...
            <CalenderWrap productid={Number(params.productid)} />
          </div>
        </div>
      </SubSectionWrap>
    </div>
