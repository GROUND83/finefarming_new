...
export async function getSlotData(productId: number) {
  let farm = await db.product.findUnique({
    where: {
      id: productId,
    },
    select: {
...
export async function getAllReservationDate(productId: number) {
...
export async function getReservationDate({
  ...
  let { slot } = (await db.product.findUnique({
    where: {
      id: productid,
    },
    select: {
...
export async function updateCalender(formData: FormData) {
  return new Promise(async (resolve, reject) => {
    ...
    console.log(productId, "date", date, newdate, newKoreanDate, plusDay);
    let dataParser = await JSON.parse(data);
    console.log("dataparser", dataParser, typeof dataParser);
