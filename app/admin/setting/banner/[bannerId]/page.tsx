...
import { deleteBanner, editFarmItems, getBanner } from "../table/actions";
import { FormTitle } from "@/app/admin/_component/form/form";
import { useRouter } from "next/navigation";

export default function Page({ params }: { params: { bannerId: string } }) {
  const [updateloading, setUpdateLoading] = React.useState(false);
  const [deleteLoading, setDeleteLoading] = React.useState(false);
  const router = useRouter();
  const form = useForm<bannerShemaType>({
  ...
  //
  const reload = async () => {
    const response = await getBanner(Number(params.bannerId));
  ...
  const clickDelete = async () => {
    try {
      ...
      await deleteBanner(Number(params.bannerId));
    } catch (e: any) {
      //
      toast.error(e);
    } finally {
    ...
    // console.log(bannerForms.fields[value]);
  };
  return (
            ...
            <div className="flex flex-row items-center justify-end gap-6 w-full">
              <div className=" col-span-6 flex flex-col items-end">
                <DeleteButton
                  ...
                  title={"배너 삭제"}
                  description={"배너 삭제 후 이벤트를 관리하세요."}
